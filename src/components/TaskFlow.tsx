...
function useTasks(projectId: string | null) {
    ...
    const update = async (id: string, data: Partial<Task> & { assigneeId?: string | null }) => {
                                        ...
                                        {sectionTasks.map(task => (
                                            <ListRow
                                                ...
                                                onMove={async (section) => {
                                                    ...
                                                    if (selectedTask?.id === task.id) setSelectedTask(prev => prev ? {
                                                        ...prev,
                                                        section,
                                                        done: section === "Done"
                                                    } : null);
            ...
            {selectedTask && project && (
                <div style={{
                    ...
                    {(row as { icon?: string }).icon ? (row as { icon?: string }).icon + " " : ""}{row.val}
                        ...
                        <div style={{
                            ...
                            <input
                                ...
                                onChange={async e => {
                                    ...
                                    await updateTask(selectedTask.id, {due});
                        ...
                        {(() => {
                            const currentAssignee = selectedTask.assignee;
                            const initials = currentAssignee?.name
                ...
                {currentAssignee.name ?? currentAssignee.email}
                                    ...
                                    {assigneeOpen && (
                                        <div style={{
                                            ...
                                            <button
                                                onClick={async () => {
                                                    await updateTask(selectedTask.id, {assigneeId: null});
                                            ...
                                            {(project.members ?? []).map(member => {
                                                const ini = member.user.name
                                                    ? member.user.name.split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 2)
                                                    : member.user.email[0].toUpperCase();
                                                const isActive = selectedTask.assigneeId === member.user.id;
                                                return (
                                                    <button
                                                        key={member.user.id}
                                                        onClick={async () => {
                                                            await updateTask(selectedTask.id, {assigneeId: member.user.id});
                                                            setSelectedTask(prev => prev ? {
                                                                ...prev,
                                                        ...
                                                        onMouseEnter={e => {
                                                            if (!isActive) (e.currentTarget.style.background = "#2d2d3a");
                                                        }}
                                                        onMouseLeave={e => {
                                ...
                                {["Low", "Medium", "High"].map(p => {
                                    ...
                                    return (
                                        <button
                                            key={p}
                                            onClick={async () => {
                                                await updateTask(selectedTask.id, {priority: p});
                        ...
                        <div style={{marginTop: 20}}>
                            <div
                                style={{fontSize: 12, fontWeight: 600, color: "#64748b", marginBottom: 10}}>DESCRIPTION
                            </div>
                            <textarea
